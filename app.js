...
const main = async () => {
	...
	io.on("connection", (socket) => {
		...
		socket.on("chat:send", async ({userId,username,type,text,url,lat,long}) => {
			...
			axios.post(`${endpoint}/api/chat/add-message`, {user_id: userId,username,type,text,url,lat,long,village_id: user.room})
				.then(res => {
		...
		socket.on("currentLocation", ({user_id, lat, long}) => {
			...
			axios.post(`${endpoint}/api/current-location`, {user_id, lat, long})
			.then(res => {
